...
__all__ = ["OVDFINETransformer"]
     
class OVDFINETransformerDecoder(nn.Module):   
    ...
    def forward(  
        ...
        for i, layer in enumerate(self.layers):    
            ...
            if i == 0:     
                ...
                pre_scores = None if not self.training and isinstance(score_head[0], nn.Identity) \
                    else self._run_score_head(score_head[0], output, text_feats)
...
@register()   
class OVDFINETransformer(DFINETransformer): 
    ...
    def __init__(   
        ...
        assert query_select_method in ("default", "one2many"), "agnostic query selection is not supported in OVDFINE"   
        super().__init__(
        ...
        self.decoder = OVDFINETransformerDecoder(self.decoder)  
    ...
    def forward(self, feats, targets=None, text_feats=None):   
        if text_feats is None: 
            raise RuntimeError("OVDFINETransformer requires text_feats.")
