...
class FDPN(nn.Module): 
    def __init__(self, 
        ...
        self.p5_block2 = C2f(hidden_dim * 2, hidden_dim, round(3 * depth_mult), shortcut=True)
    
        if len(out_strides) == 3: 
            self.p4_to_p3_up2 = nn.Upsample(scale_factor=2)     
            self.p3_block2 = C2f(hidden_dim * 2, hidden_dim, round(3 * depth_mult), shortcut=True)
    ...
    def forward(self, feats):   
        ...
        fouce_feature2_to_p5 = self.p5_block2(torch.cat([fouce_feature2_to_p5, fouce_feature1_to_p5_2], dim=1))

        if len(self.out_strides) == 3:
            fouce_feature2_to_p3 = self.p4_to_p3_up2(fouce_feature2) # fouce_feature2 to p3   
            fouce_feature2_to_p3 = self.p3_block2(torch.cat([fouce_feature2_to_p3, fouce_feature1_to_p3_2], dim=1)) 
