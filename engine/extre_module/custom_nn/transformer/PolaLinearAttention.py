...
class PolaLinearAttention(nn.Module):
    ...
    def _to_nlc(self, x):   
        if x.dim() == 4:  
            ...
            if (h, w) != (self.h, self.w):   
                raise ValueError(     
                    f"PolaLinearAttention was initialized for hw={(self.h, self.w)}, got {(h, w)}"    
                ) 
            return x.flatten(2).transpose(1, 2).contiguous(), (b, c, h, w)  
        if x.dim() == 3:
            return x, None
        raise ValueError(f"PolaLinearAttention expects BCHW or BNC input, got {tuple(x.shape)}")   
    ...
    def forward(self, x):
        ...
        if N != self.h * self.w:
            raise ValueError(    
                f"PolaLinearAttention was initialized for {self.h * self.w} tokens, got {N}"   
            )     
        ...
        return self._restore_layout(x, bchw_shape)     
