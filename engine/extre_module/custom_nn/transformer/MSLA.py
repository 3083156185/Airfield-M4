...
class MSLA(nn.Module):
    ...
    def _to_bchw(x):   
        ...
        if x.dim() == 3:
            ...
            h = w = int(n ** 0.5)
            if h * w != n:     
                raise ValueError(f"MSLA expects a square token map, got {n} tokens")
            return x.transpose(1, 2).reshape(b, c, h, w).contiguous(), (b, n, c)
        raise ValueError(f"MSLA expects BCHW or BNC input, got {tuple(x.shape)}")

    @staticmethod    
    def _restore_layout(x, nlc_shape):     
        if nlc_shape is None:
            return x
        b, n, _ = nlc_shape
        return x.flatten(2).transpose(1, 2).reshape(b, n, -1).contiguous()    

    def forward(self, input_):     
        input_reshaped, nlc_shape = self._to_bchw(input_)   
        _, c, _, _ = input_reshaped.shape
        ...
        return self._restore_layout(final_output, nlc_shape)
